...
class PublicSuffix {
    ...
    private fun findMatchingRule(domainLabels: List<String>): List<String> {
        ...
        val domainLabelsUtf8Bytes =
            Array(domainLabels.size) { i -> domainLabels[i].toByteArray(UTF_8) }
    ...
    private fun readTheList() {
        ...
        val resource =
            PublicSuffix::class.java.getResourceAsStream(PUBLIC_SUFFIX_RESOURCE) ?: return
        ...
        GzipSource(resource.source()).buffer().use { bufferedSource ->
            ...
            publicSuffixExceptionListBytes =
                bufferedSource.readByteArray(totalExceptionBytes.toLong())
        }
