...
`scripts/tests/cursor_tests.py` holds the Android TV **cursor** tests (see the
...
- **`CursorController`** — owns cursor position/velocity, the `Choreographer` movement loop,
  ...
  (the current WebView), a `CursorSettings` interface and an `onCursorToggled` callback — no
...
`isCursorAvailable()` / `isCursorOn()` / `toggleCursor()`, wires the menu item and
...
- **Context menu = a *deliberate* (~1 s) hold of the action key.** While the cursor is on
  ...
  `isConfirmKey`; BUTTON_A resolves to DPAD_CENTER on most remotes) for a deliberate hold —
  `settings.actionHoldSec`, the user-configurable `pref_key_cursor_action_hold` (0.5-1.0 s, default
  1.0 s; the controller clamps it to at least `minActionHoldSec` = 0.5 s) — performs a synthetic
  touch long press at the cursor point (`dispatchLongPress`:
...
- **Toggle preferences use a dynamic summary** (per the Material Design
  guidelines): set `app:summaryOn` / `app:summaryOff` (or `setSummaryOn` /
  `setSummaryOff` in code) so the summary reflects the current state (different
  wording per state — e.g. what the feature does when on, how to get it when off)
  rather than being a static description. See the cursor hotkey in
